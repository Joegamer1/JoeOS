...
export function ContentView({ section, onOpen }: { section: ContentId; onOpen?: (id: AppId) => void }) {
  ...
  return <article className={`content-view content-${section}`}>
    <div className="document-eyebrow">{section === "about" ? "A note of introduction" : section === "projects" ? "Things I'm building" : section === "blog" ? "Personal & technical writing" : "Engineering notes"}</div>
    <h1>{heading}</h1>
    {section === "about" ? <>
      ...
      <p className="page-summary">{page.summary}</p><section className="project-entry" aria-labelledby="joeos-project"><div className="project-heading"><h2 id="joeos-project">JoeOS</h2><span>In progress</span></div><p>{page.paragraphs[0]}</p><a href="https://github.com/Joegamer1/JoeOS">Source & construction notes <ArrowUpRight size={16} aria-hidden="true" /></a></section><p className="quiet-note">{page.paragraphs[1]}</p>
    </> : section === "blog" ? <>
      ...
      <p className="intro-text">{page.paragraphs[0]}</p><div className="case-study-note"><h2>Stories about the work.</h2><p>{page.paragraphs[1]}</p></div><p className="quiet-note">{page.paragraphs[2]}</p>
    </>}
  </article>;
}
